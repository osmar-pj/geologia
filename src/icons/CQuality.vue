<template>
  <svg
    xmlns="http://www.w3.org/2000/svg"
    width="24"
    height="24"
    viewBox="0 0 24 24"
    fill="currentColor"
    class="injectedSvg"
    dataSrc="/icons/artificialIntelligence07StrokeRounded.svg"
    xmlns:xlink="http://www.w3.org/1999/xlink"
    role="img"
    color="#000000"
  >
    <path
      d="M13 2C12.2832 2 11.414 2.00014 11.1442 2.18897C10.8744 2.3778 10.7611 2.70794 10.5344 3.36821L10.1897 4.36598C10.0319 4.76472 9.74679 5.10395 9.37612 5.33402L9.02837 5.52847C8.59612 5.7437 8.09945 5.80372 7.62605 5.69792L6.36645 5.35409C5.62494 5.15017 5.25419 5.04821 4.92637 5.17743C4.59855 5.30664 4.40588 5.63068 4.02055 6.27876L3.51103 7.13568C3.14984 7.74316 2.96924 8.0469 3.00429 8.37025C3.03934 8.69359 3.28111 8.95416 3.76466 9.4753L4.82896 10.6284C5.08908 10.9476 5.27376 11.5038 5.27376 12.0038C5.27376 12.5041 5.08914 13.0602 4.82898 13.3794L3.76466 14.5325C3.28112 15.0537 3.03935 15.3142 3.00429 15.6376C2.96924 15.9609 3.14984 16.2647 3.51104 16.8721L4.02053 17.729C4.40586 18.3771 4.59855 18.7012 4.92637 18.8304C5.2542 18.9596 5.62495 18.8576 6.36647 18.6537L7.626 18.3099C8.09949 18.204 8.59625 18.2641 9.02856 18.4794L9.37626 18.6739C9.74685 18.904 10.0319 19.2431 10.1897 19.6418L10.5344 20.6397C10.7611 21.3 10.8744 21.6301 11.1442 21.8189C11.414 22.0078 12.2832 22 13 22"
      stroke="currentColor"
      strokeWidth="1.5"
      strokeLinecap="round"
    ></path>
    <path
      d="M13 15.5C11.067 15.5 9.5 13.933 9.5 12C9.5 10.067 11.067 8.5 13 8.5"
      stroke="currentColor"
      strokeWidth="1.5"
      strokeLinecap="round"
    ></path>
    <path
      d="M20.25 4.75L18 7H16M19.5 4.75C19.5 5.16421 19.8358 5.5 20.25 5.5C20.6642 5.5 21 5.16421 21 4.75C21 4.33579 20.6642 4 20.25 4C19.8358 4 19.5 4.33579 19.5 4.75Z"
      stroke="currentColor"
      strokeWidth="1.5"
      strokeLinecap="round"
      strokeLinejoin="round"
    ></path>
    <path
      d="M20.25 19.25L18 17H16M19.5 19.25C19.5 18.8358 19.8358 18.5 20.25 18.5C20.6642 18.5 21 18.8358 21 19.25C21 19.6642 20.6642 20 20.25 20C19.8358 20 19.5 19.6642 19.5 19.25Z"
      stroke="currentColor"
      strokeWidth="1.5"
      strokeLinecap="round"
      strokeLinejoin="round"
    ></path>
    <path
      d="M20.25 12H16M19.5 12C19.5 12.4142 19.8358 12.75 20.25 12.75C20.6642 12.75 21 12.4142 21 12C21 11.5858 20.6642 11.25 20.25 11.25C19.8358 11.25 19.5 11.5858 19.5 12Z"
      stroke="currentColor"
      strokeWidth="1.5"
      strokeLinecap="round"
      strokeLinejoin="round"
    ></path>
  </svg>
</template>

<script>
export default {
  name: "CQuality",
  props: {
    color: {
      type: String,
      default: "#fff", // Valor por defecto, puedes ajustarlo según tus necesidades
    },
  },
};
</script>
